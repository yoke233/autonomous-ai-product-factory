...
首个实现靶场收窄为：**单一后端仓库**（具体目标仓库与语言栈在 M0 启动时选定并固定为 Baseline 的一部分）、隔离 branch/worktree、自动构建与受保护验证，主交付模式为 `STAGING`。交付自主度按证据阶梯解锁：
...
| 阶段 | 范围 | 退出条件 |
|---|---|---|
| M0 评测基线 | 选定首个靶场仓库；从其历史选取任务、正确 Patch、测试和已知失败样本；校验受保护测试集自身质量（mutation 分数 + 正确/错误样本双向校验，INV-13）；用标注歧义样本校准分级歧义门 | 同一输入可重放；每个判断可追溯 Evidence；能模拟 stale Baseline、Runner 中断、权限撤销和测试篡改；歧义门的精度/召回有基线数字 |
