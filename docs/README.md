...
系统适配任意 Git 仓库，不绑定靶场：每个 Goal 引用一个仓库，其语言栈、构建/测试/部署命令由 **Repo Profile** 声明并随 Baseline Snapshot 冻结；执行统一使用隔离 branch/worktree、自动构建与受保护验证，主交付模式为 `STAGING`。操作入口是本地 **Web Console**（GoalIngressPort + OutcomeProjectionPort 的第一个 Connector，见 [04-integrations.md](04-integrations.md)），飞书等连接器后置。交付自主度按证据阶梯解锁：
